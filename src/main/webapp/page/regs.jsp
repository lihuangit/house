...
$(function () {
    $("#name").blur(function () {
        $.post("/selectOne",{"name":$("#name").val()},function (data) {
            if(data.result>0)
                $("#span").html("该用户已存在").css("color","red")
            else
                $("#span").html("该用户可以用").css("color","green")
...
<TABLE class=field>
  ...
  <TR>
    ...
    <TD><INPUT class=text type=text name=name id="name"><span id="span"></span> </TD></TR>
  <TR>
