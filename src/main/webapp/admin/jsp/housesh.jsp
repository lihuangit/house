        ...
        $('#dg').datagrid({
            ...
            columns: [[
                ...
                { field: 'caozuo', title: '操作', width: 50, align: "center",formatter:
                        function(value,row,index){
                            return "<a href='javascript:quxiaosh("+row.id+")'>取消审核</a>"
                        }
                }
            ]]
        });
...
function quxiaosh(id) {
    $.post("/quxiaosh",{"id":id},function (data) {
        if (data.result > 0) {
            $.messager.alert("提示框","取消成功!")
            $("#dg").datagrid("reload")
        }else {
            $.messager.alert("提示框","取消失败!")
        }
    },"json")
}
function piliang() {
    ...
    $.post("/piliangquxiaosh",{"ids":str},function (data) {
        if (data.result>0){
            $.messager.alert("提示框","批量取消成功!")
            $("#dg").datagrid("reload");
        } else {
            $.messager.alert("提示框","批量取消失败!")
        }
    },"json")
}
    function sousuo() {
...
<div id="ToolBar">
    <a
            href="javascript:piliang()" class="easyui-linkbutton"
            iconCls="icon-remove" plain="true">批量取消审核</a>
    </div>
